...
function riderIcon(bearing: number) {
  return L.divIcon({
    ...
    html: `<div style="position:relative;width:46px;height:46px;">
      <div style="position:absolute;inset:0;border-radius:50%;background:#f59e0b;opacity:.22;animation:pulse 1.5s infinite;"></div>
      <svg width="46" height="46" viewBox="0 0 46 46" fill="none" style="position:absolute;inset:0;filter:drop-shadow(0 2px 8px rgba(245,158,11,.6))">
        <circle cx="23" cy="23" r="19" fill="#1e293b" stroke="#f59e0b" stroke-width="2.5"/>
        <g transform="rotate(${bearing}, 23, 23)">
          <path d="M23 7 L31 33 L23 27 L15 33 Z"
            fill="#f59e0b" stroke="#fff" stroke-width="1.8" stroke-linejoin="round"/>
        </g>
      </svg>
    </div>`,
    iconAnchor: [23, 23],
...
export default function MapView({
  entries, gaps, selectedIndex, onSelectEntry,
  riderId, selectedGap, playbackIndex,
  ...
  useEffect(() => {
    ...
    const map = L.map(containerRef.current, { zoomControl: false }).setView([23.7426, 90.3824], 16);
  ...
  useEffect(() => {
    ...
    const glowLine = L.polyline(latlngs, { color: "#7dd3fc", weight: 12, opacity: 0.18 }).addTo(map);
    const polyline = L.polyline(latlngs, { color: "#38bdf8", weight: 4, opacity: 0.85 }).addTo(map);

    const newMarkers: L.Layer[] = [];

    newMarkers.push(
      L.marker(latlngs[0], { icon: START_ICON }).addTo(map).bindPopup(
    ...
    newMarkers.push(
      L.marker(latlngs[latlngs.length - 1], { icon: END_ICON }).addTo(map).bindPopup(
    ...
    entries.forEach((e, i) => {
      newMarkers.push(
        L.circleMarker([e.lat, e.lng], { radius: 5, color: "transparent", fillOpacity: 0 })
          .addTo(map)
    ...
    const newGapMarkers: L.Layer[] = gaps
      .map((g) => {
        ...
        return L.marker([fromEntry.lat, fromEntry.lng], { icon: GAP_ICON })
          .addTo(map)
          .bindPopup(`<b>⚠ Signal Gap</b><br>Duration: ${humanDuration(g.durationMs)}<br>Estimated missed: ${g.missedCount}`);
      })
    ...
    map.invalidateSize();
    setTimeout(() => map.fitBounds(polyline.getBounds(), { padding: [50, 50] }), 50);
  }, [entries, gaps]);
  ...
  useEffect(() => {
    ...
    const marker = L.circleMarker([e.lat, e.lng], {
      radius: 9, color: "#f59e0b", fillColor: "#fde68a", fillOpacity: 0.9, weight: 2,
    })
      .addTo(map)
      .bindPopup(
  ...
  useEffect(() => {
    ...
    const beforeEntry = [...entries].reverse().find((e) => e.timestamp <= selectedGap.from);
    ...
    layers.push(
      L.polyline([[beforeEntry.lat, beforeEntry.lng], [afterEntry.lat, afterEntry.lng]], {
        color: "#f97316", weight: 3, dashArray: "8 6", opacity: 0.9,
      }).addTo(map)
    );
    layers.push(
      L.circleMarker([beforeEntry.lat, beforeEntry.lng], {
        radius: 11, color: "#f97316", fillColor: "#fed7aa", fillOpacity: 0.95, weight: 2,
      }).addTo(map).bindPopup(
    ...
    layers.push(
      L.circleMarker([afterEntry.lat, afterEntry.lng], {
        radius: 11, color: "#eab308", fillColor: "#fef08a", fillOpacity: 0.95, weight: 2,
      }).addTo(map).bindPopup(
    ...
    map.fitBounds(
      L.latLngBounds([beforeEntry.lat, beforeEntry.lng], [afterEntry.lat, afterEntry.lng]),
  ...
  useEffect(() => {
    ...
    if (layersRef.current.traveledGlow) map?.removeLayer(layersRef.current.traveledGlow);
    if (layersRef.current.traveledPath) map?.removeLayer(layersRef.current.traveledPath);
    ...
    if (playbackIndex > 0) {
      const latlngs: L.LatLngTuple[] = entries.slice(0, playbackIndex + 1).map((e) => [e.lat, e.lng]);
      layersRef.current.traveledGlow = L.polyline(latlngs, {
        color: "#fb923c", weight: 14, opacity: 0.22,
      }).addTo(map);
      layersRef.current.traveledPath = L.polyline(latlngs, {
        color: "#f97316", weight: 5, opacity: 1,
      }).addTo(map);
    }

    const e = entries[playbackIndex];
    const marker = L.marker([e.lat, e.lng], { icon: riderIcon(e.bearing), zIndexOffset: 1000 })
      .addTo(map)
      .bindPopup(
  ...
  useEffect(() => {
    ...
    const newMarkers: L.Layer[] = customLocations.map((loc) => {
      ...
      const marker = L.marker([loc.lat, loc.lng], { icon: customLocationIcon(loc.color, loc.name, isSelected) })
        .addTo(map)
        .bindPopup(
  ...
  return (
    <div style={{
      ...
      <div ref={containerRef} style={{ height: "100%", width: "100%", borderRadius: isFullscreen ? 0 : "8px" }} />
    </div>
  );
}
